...
import zed.parser.Grammar.Drop;
import zed.parser.Grammar.Lift;
import zed.parser.Grammar.Nothing;
...
public class ParseTreeToAST {
    ...
    public static ASTNode convertToAST(Memo memo, String input) {
        if (memo.isRule("Ident") || memo.isRule("Match")) {
            return new IdentNode(input.substring(memo.startIdx, memo.endIdx));

        } else if (memo.expr instanceof Lift) {
        ...
        } else if (memo.expr instanceof Drop || memo.expr instanceof Nothing) {
            return null;
        }
